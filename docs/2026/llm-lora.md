...
在大模型的每层中，有Q/K/V/O几个矩阵，这些矩阵的行数和列数都和向量维度有关，例如在Qwen3-0.6B中是1024‌×1024和1024‌×2048，每个矩阵的参数量都非常大。我们来看一下公式，这里以1024‌×1024为例说明。
...
但LoRA方法，将些矩阵冻结不修改，而是在旁边挂AB两个小矩阵，通过调整两个小矩阵的参数值来影响最终结果。
...
其中a和r为超参数，超参数的含义是我们在训练之前就提前确定好的参数，训练过程中不会变化。r的含义
