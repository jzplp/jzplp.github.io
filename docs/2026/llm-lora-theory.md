...
\begin{align*}
设：&\theta_{old}为当前参数值；\theta_{new}为新参数值；\eta 为学习率 \\
&v_{old}为当前“速度”，v_{new}为新速度 \\
&\mu 为动量系数，g为参数梯度，即\left.\frac{\partial L}{\partial \theta}\right|_{\theta=\theta_{old}}的简写
\\
&v_{new} = \mu v_{old} + g \\
&\theta_{new} = \theta_{old} - \eta v_{new}
\end{align*}
$$
...
$$
\begin{align*}
&设：\eta 学习率为0.001；\mu 动量系数为0.9；g参数梯度固定为0.1\\
&v_1 = 0 + 0.1 = 0.1 \quad \theta_1 = 0 - 0.001\times 0.1 = -0.0001 \\
&v_2 = 0.1\times 0.9 + 0.1 = 0.19 \quad \theta_2 = -0.0001 - 0.001\times 0.19 = -0.00029 \\
&v_3 = 0.19\times 0.9 + 0.1 = 0.271 \quad \theta_3 = -0.00029 - 0.001\times 0.271 = -0.000561 \\
&v_4  = 0.3439 \quad \theta_4 = -0.0009049 \\
&...
\end{align*}
$$
...
\begin{align*}
&v_{t+1} = 0.9v_{t} + g \\
&当步数趋近于无穷时，假设存在一个稳定值v_{max} \\
&v_{max} = 0.9v_{max} + g \\
&v_{max} = \frac{g}{1-0.9} = 10g  \\
\end{align*}
$$
...
$$
\begin{align*}
&设：\eta 学习率为0.001；\mu 动量系数为0.9；g参数梯度为10,-8,6,-4等\\
&v_1 = 0.9\times 0 + 10 = 10 \quad \theta_1 = 0 - 0.001\times 10 = -0.01 \\
&v_2 = 0.9\times 10 - 8 = 1 \quad \theta_2 = -0.01 - 0.001\times 1 = -0.011 \\
&v_3 = 0.9\times 1 + 6 = 6.9 \quad \theta_3 = -0.011 - 0.001\times 6.9 = -0.0179 \\
&v_4 = 2.21 \quad \theta_4 = -0.02011 \\
&...\\
&对比原始梯度下降法：\\
&\theta_1 = 0 - 0.001\times 10 = -0.01 \\
&\theta_2 = -0.01 - 0.001\times (-8) = -0.002 \\
&\theta_3 = -0.002 - 0.001\times 6 = -0.008 \\
&\theta_4 = -0.008 - 0.001\times (-4) = -0.004 \\
&...\\
\end{align*}
$$
...
$$
\begin{align*}
&设：\theta_{t}为第t步的参数值，每个参数分别计算；\eta 为学习率 \\
&G_t为历史梯度值的平方相加，g为参数梯度 \\
&\epsilon 为防止除数为0的超参数，一般为 10^{-8}\\
\\
&G_{t+1} = G_{t} + g^2 \\
&\theta_{t+1} = \theta_t - \frac{\eta}{\sqrt{G_{t+1}} + \epsilon}g
\end{align*}
$$
...
$$
\begin{align*}
&g_1 = 1 \quad G_1 = 1^2 = 1 \quad \sqrt{G_1} = 1\\
&g_2 = 10 \quad G_2 = 1 + 10^2 = 101 \quad \sqrt{G_2} \approx  10.05 \\
&g_3 = 100 \quad G_3 = 101 + 100^2 = 10101 \quad \sqrt{G_3} \approx 1005 \\
\end{align*}
$$
...
\begin{align*}
&设：\theta_{t}为第t步的参数值，每个参数分别计算；\eta 为学习率 \\
&G_t为历史梯度的平方加权平均；\beta为权重，一般为0.9或0.99 \\
&g为参数梯度，\epsilon 为防止除数为0的超参数，一般为 10^{-8}\\
\\
...
- 深度学习基础理论————学习率优化方法(AdaGrad/RMSprop/Adam/Warm-UP)\
  https://www.cnblogs.com/Big-Yellow/p/18615545
- 深度学习中常用优化器的总结\
  https://zhuanlan.zhihu.com/p/166362509
