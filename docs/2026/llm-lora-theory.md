...
\theta_{new} &= \theta_{old} - \left.\eta\frac{\partial }{\partial \theta}\right|_{\theta=\theta_{old}}
\end{align*}
$$
...
* SGD 随机梯度下降 Stochastic Gradient Descent
  * 一个样本就更新一次参数值
* MBGD 批量梯度下降 Batch Gradient Descent
  * 多个样本组合为一个batch，一个batch完成后计算一次梯度值
* BGD 小批量梯度下降 Mini-Batch Gradient Descent
  * 所有训练样本完成后再计算一次梯度值
