...
可以看到，通过简单的两句代码即可加载模型。其中transformers内置了很多开源大模型的适配工具，它会识别模型配置文件，自动选择合适的模型处理方式。输出结果如下：
...
注意这里分词器将模型思考内容`<think></think>`省略了。因此思考内容太长且保存意义不大。而且发现我们的内容前后有一些特殊符号，这些是模型的对话模板格式，有消息开始/结束标记，角色标记等。不同的模型对话模板是不一致的。然后就是真正的分词，将文本分割为一个一个的token，然后转化为TokenID。输出outputs内容如下：
