...
### K-Quants方法
GGUF格式有一个专用的量化方式，叫做K-Quants方法。它将256个数字分为一个超级块，使用两级缩放因子，达到尽量节约空间和减小误差的效果。这里我们以Q4_K为例介绍一下：
...
* K-Quants方法  256*4 + 8*6*2 + 16*2 = 1152 bits

以K-Quants方法量化的格式，命名叫做QX_K，其中X表示单个参数存储的位数，例如Q2_K, Q4_K等等。QX_K后面还能加后缀，可以将模型按照重要程度使用不同的位数存储，例如使用Q6或者Q8存储，即混合精度：
