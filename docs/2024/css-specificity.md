...
`!important`的优先级说明：
- 带`!important`标志与不带标志的比较，带标志的优先级更高。
- 正在过渡的样式，优先级比带`!important`标志的优先级更高。
- 同样带`!important`标志的属性互相比较，按照原有的优先级比较方式比较。
- 作者样式，用户自定义样式，用户代理样式表相比较：
  - 同样不带`!important`标志，优先级为 作者样式 > 用户自定义样式 > 用户代理样式表
  - 存在一个带`!important`标志，优先级为 带`!important`标志的更高
  - 存在多个带`!important`标志，那么`!important`标志的进行比较。优先级为 用户代理`!impoortant`样式表 > 用户自定义`!impoortant`样式 > 作者`!impoortant`样式
