...
答案都是否：没有进位规则；而且低优先级的选择器无论出现多少个，优先级也不会比高优先级的选择器更高。
...
通用选择器是可以选择所有元素的选择器，优先级为 0-0-0。这个优先级实际上比上面介绍的任何选择器的优先级更低。
...
关于选择器的优先级，有两个大原则：
1. 选择器越“精确”，优先级越高。
2. 判断选择器的优先级，要根据它的具体功能来判断。
（todo 还有没有其他原则）

这些原则不仅在这里适用，对于后面提到的更多优先级规则也适用。
