...
`:is()` 接受一个可容错的选择器列表，例如`:is(p, .a, #b)`。如果列表中任意选择器命中，则该伪类命中。这个伪类的优先级规则和并集选择器一致：是由选择器列表中生效的优先级最高的选择器决定该选择器的优先级。
...
### `:where()`
`:where()`同样是一个函数式伪类，也接受一个选择器列表做入参，但是优先级规则却与上面的函数式伪类不一样，因此单独描述。
