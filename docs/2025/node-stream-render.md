...
## 总结
流式渲染在实际应用中是一个很常见也很有用的需求，通过流式渲染页面可以尽快响应，页面的白屏时间会缩短。甚至有一个专门的指标TTFB(Time To First Byte)用于衡量从请求HTML开始到接收到响应的第一个字节之间的时间。例如在前端SSR中，数据会在Node端进行接口请求，拿到数据后组装成HTML后再发给前端。这时需要消耗较长时间的，如果使用流式渲染提前渲染部分准备好的页面内容，用户体验会好很多。

在流式传输方面，我们常见也是目前最流行的就是AI问答了，AI的回答并不是一口气全部生成完成的，而是分块的。如果等所有数据生成完在战时需要等很久。利用流式传输分块传送，可以将生成的部分内容传送到到客户端，用户先看一部分，这样也提高了用户体验。因此现在基本上所有AI问答看起来都是一个字一个字往外蹦的。
