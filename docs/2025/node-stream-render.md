...
我们日常使用流式体验最深的应该是观看在线高清视频：高清视频是数据量非常大的，如果全部下载再观看，需要耗费很长时间。大部分网站都会采用流式传输技术，把大视频分割成很多个小数据块分别发送。这些小数据块在客户端进行处理后就可以播放了，不需要等所有块都接收后再播放。

前端页面的流式渲染和数据使用的流式传输在原理上与流式视频类似。只不过流式渲染是将HTML页面分块发送，流式传输是将服务端数据分块发送。这里画出了流式与非流式在传输和渲染模式的对比流程图：

![图片](/2025/stream-1.png)

![图片](/2025/stream-2.png)

通过流程图可以清晰的看到，流式的优势在于可以服务端准备好一部分数据就发给前端进行展示，不必把所有数据都准备好再发送，前端的页面或数据展示可以和服务端处理并行进行。对于有需求的场景，用户体验和速度都能提升。
...
在Node.js中，使用`res.write`方法，在body上发送一个数据块。可以多次发送数据，接口传输不会停止，直到使用`res.end`方法结束发送。`res.write`方法会在header中自动增加`Transfer-Encoding: chunked`表示分块传输。使用这个方式，即可实现Node.js的流式传输。
...
这次服务端代码分为了两部分，匹配`url === "/"`的部分返回固定的字符串作为页面的HTML；匹配`url === "/api/stream"`的部分定时返回流式数据。虽然流式数据仅仅是字符串（与最开始流式传输的举例一致）但这次浏览器可以正常接收流式数据了。我们看下gif动图：
...
HTTP报文的Header和Body部分都使用帧来传送，其中Header部分为单独的一个帧，Body部分可以分为多个帧独立传送。而且HTTP2实现了多路复用，同一个地址的不同请求使用同一个TCP连接，避免了HTTP1.1限制最大连接数的问题（后面总结部分会描述这个问题）。
...
## 总结
流式渲染在实际应用中是一个很常见也很有用的需求，通过流式渲染页面可以尽快响应，页面的白屏时间会缩短。甚至有一个专门的指标TTFB(Time To First Byte)用于衡量从请求HTML开始到接收到响应的第一个字节之间的时间。例如在前端SSR中，数据会在Node端进行接口请求，拿到数据后组装成HTML后再发给前端。这是需要消耗较长时间的，如果使用流式渲染技术，提前渲染部分准备好的页面内容，用户体验会好很多。

在流式传输方面，我们常见也是目前最流行的就是AI问答了，AI的回答并不是一口气全部生成完成的，而是分块的。如果等所有数据生成完再展示需要等很长时间。利用流式传输分块传送，可以将生成的部分内容传送到到客户端，用户先看一部分，这样也提高了用户体验。因此现在基本上所有AI问答看起来都是一个字一个字往外蹦的。

当然流式也并不全是优势。流式会占用长时间TCP连接，而浏览器对于一个页面TCP连接总数是有限制的，例如部分浏览器可能只有6个。如果流式接口较多，会影响其它数据的传输。当然这一点在HTTP2中，通过多路复用技术已经解决了。

关于流式渲染和传输的相关主题还有很多，例如Node.js实现流的原理；React和Vue等页面框架是如何支持流式；Blob与FileReader等流式对象；Axios等如何支持流式传输；JavaScript的二进制缓冲区等等。不想把文章写的超级长，这里就不展开太多主题了。
