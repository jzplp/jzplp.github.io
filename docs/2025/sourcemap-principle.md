...
### Base64编码
我们很久之前介绍过Base64编码，那时候是将3个字节转换为4个Base64编码值：因为 3*8=24, 6*4=24，位数相同正好能转换。但是这里由于我们在VLQ编码的时候就已经设置了6位，因此就不需要合并字节了，而是直接把一组二进制数转为对应字符即可：
